...
**清理（VACUUM）**是一种维护过程，有助于PostgreSQL的持久运行。它的两个主要任务是，删除死元组和冻结事务ID，两者都会在5.10节中简要提到。
...
​	由于清理过程涉及到整个表的扫描，因此该过程开销显著。在版本8.4（2009）中，引入了**可见性映射（Visibility Map, VM）**来提高移除死元组的效率。在版本9.6（2016）中通过增强了VM以改进冻结过程。
...
​	该伪码有两个部分：一个依次处理表的循环部分，以及后处理部分。而内部的循环又能分为三个部分，每一个部分都有各自的任务。接下来会描述这三个部分，以及后处理的轮廓。
...
此外，如果最后一页没有元组，则会从表文件中被截断。

### 6.1.4 后处理

​	当处理完成后，PostgreSQL会更新与清理过程相关的几个统计数据和系统视图，如果可能的话，它还会移除clog中不必要的部分（第6.4节）。
...
​	第5.4节中描述的clog存储了事务状态。 当更新`pg_database.datfrozenxid`时，PostgreSQL会尝试删除不必要的clog文件。 请注意，也会删除相应的clog页面。

​	图6.7显示了一个示例。 如果clog文件`'0002'`中包含最小的`pg_database.datfrozenxid`，则可以删除旧文件（`0000`和`0001`），因为存储在这些文件中的所有事务都可以在整个数据库集群中视为冻结的txids。
...
​	清理过程已通过**自动清理（AutoVacuum）**守护进程自动化；因此运维PostgreSQL变得容易了一些。

​	autovacuum守护程序定期调用几个`autovacuum_worker`进程。默认情况下，它每1分钟唤醒一次（由`autovacuum_naptime`定义），并调用三个worker（由`autovacuum_max_works`定义）。

autovacuum调用的autovacuum worker逐步对各个表执行VACUUM处理，对数据库活动的影响最小。
...
​	当执行完整清理时，没有人可以访问（读/写）表。最多两倍于表的磁盘空间会被临时使用；因此在处理大表时，有必要检查剩余磁盘容量。

> ### 什么时候该使用VACUUM FULL？
>
> ​	遗憾的是当执行`VACUUM FULL`时，没有最佳实践。但是，扩展`pg_freespacemap`可能会给你很好的建议。
>
