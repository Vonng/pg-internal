...
* 第4部分：第5.10节。
  ...
  本部分描述了永久运行并发控制机制所需的几个维护过程。维护过程通过真空处理进行，如第6章所述。
  本章重点介绍PostgreSQL独有的主题，尽管有许多与并发控制相关的主题。请注意，省略了死锁预防和锁定模式的描述（有关更多信息，请参阅官方文档）。
...
> | :-------: | :----: | :--------: | :----------------------------: | :--------: |
> | 读已提交  | 不可能 |    可能    |              可能              |    可能    |
> | 可重复读* | 不可能 |   不可能   | PG中不可能<br />但ANSI SQL允许 |    可能    |
> | 可序列化  | 不可能 |   不可能   |             不可能             |   不可能   |
>
> 在9.0及更早版本中，此级别已用作“SERIALIZABLE”，因为它不允许ANSI SQL-92标准中定义的三个异常。 但是，随着9.1版中SSI的实现，此级别已更改为“REPEATABLE READ”并引入了真正的SERIALIZABLE级别。

> PostgreSQL对DML（增删改查）使用SSI，对DDL（CREATE,DROP,etc...）使用2PL。



## 5.1 事务ID

每当事务开始时，事务管理器就会分配一个称为事务id（txid）的唯一标识符。 PostgreSQL的txid是一个32位无符号整数，大约42亿（千万）。如果在事务启动后执行内置的txid_current（）函数，该函数将返回当前的txid，如下所示。
...
Txids可以相互比较。例如，在txid 100的观点处，大于100的txids是“未来”并且它们从txid 100不可见;小于100的txids是“过去”和可见的（图5.1a））。
...
表页中的堆元组被分类为通常的数据元组和TOAST元组。本节仅介绍常用元组。
