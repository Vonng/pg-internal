---
title: 基础备份与时间点恢复
description: 基础备份、时间点恢复、时间线与时间线历史文件。
search_keywords: [base backup, PITR, point-in-time recovery, timeline, recovery.conf, backup_label]
type: book
book_kind: chapter
book_number: "10"
upstream_link: https://www.interdb.jp/pg/pgsql10/
weight: 310
breadcrumbs: false
---
...
1. 发出[`pg_start_backup`](https://www.postgresql.org/docs/current/functions-admin.html#FUNCTIONS-ADMIN-BACKUP)命令
...
更简单的方式是使用[`pg_basebackup`](https://www.postgresql.org/docs/current/app-pgbasebackup.html)命令来做基础备份，不过在它内部也是使用这些底层命令来工作的。

{{< fig num="10.1" src="/img/fig-10-01.png" caption="制作基础备份" alt="图10.1 制作基础备份" />}}
...
`pg_start_backup`开始为制作基础备份进行准备工作。如[第9.8节](/ch9/)所述，恢复过程从重做点开始，因此`pg_start_backup`必须执行检查点，以便在制作基础备份的开始时刻显式创建一个重做点。此外，这次检查点的位置必须保存在不同于`pg_control`的其他文件中，因为在备份期间可能会进行多次常规检查点。因此`pg_start_backup`执行下列四个操作：

1. 强制进入**整页写入**模式。
2. 切换到当前的WAL段文件（8.4或更高版本）。
3. 执行检查点。
4. 创建`backup_label`文件 —— 该文件创建于基本目录顶层中，包含有关该基本备份本身的关键信息，例如检查点位置。
...
{{< xref fig="10.2" anchor="fig-10.2" >}}图10.2{{< /xref >}}展示了PITR的基本概念。 PITR模式下的PostgreSQL会在基础备份上重放归档日志中的WAL数据，从`pg_start_backup`创建的重做点开始，恢复至你想要的位置为止。在PostgreSQL中，要恢复到的位置，被称为**恢复目标（recovery target）**。

{{< fig num="10.2" src="/img/fig-10-02.png" caption="PITR的基本概念" alt="图10.2 PITR的基本概念" />}}

PITR是这样工作的。假设你在GMT时间`2018-07-16 12:05:00`进行了误操作。你应该删除当前的数据库集簇，并使用之前制作的基础备份恢复一个新的集簇。然后创建`recovery.conf`文件，将参数`recovery_target_time`配置为误操作发生的时间点（在本例中，也就是12:05 GMT）。`recovery.conf`文件如下所示：
...
{{< xref fig="10.3" anchor="fig-10.3" >}}图10.3{{< /xref >}}从时间线标识的角度展示了PITR过程。首先，我们删除当前的数据库集簇，并替换为过去的基础备份，以便返回到恢复的起始点，这一步在图中用红色曲线箭头标识。接下来，我们启动PostgreSQL服务器，它通过跟踪初始时间线（时间线标识`1`），从`pg_start_backup`创建的重做点开始，重放归档日志中的WAL数据，直到恢复目标达成，这一步在图中用蓝色直线箭头标识。接下来，恢复得到的数据库集簇将被分配一个新的时间线标识`2`，而PostgreSQL将运行在新的时间线上。

{{< fig num="10.3" src="/img/fig-10-03.png" caption="原始数据库集簇和恢复数据库集簇之间时间线标识的关系" alt="图10.3 原始数据库集簇和恢复数据库集簇之间时间线标识的关系" />}}
...
$\color{blue}{00000001}0000000000000009$，以及 $\color{blue}{00000001}000000000000000A$。新恢复得到的数据库集簇被分配了时间线标识`2`，而PostgreSQL就会从 $\color{blue}{00000002}000000000000000A$ 开始创建WAL段。如{{< xref fig="10.4" anchor="fig-10.4" >}}图10.4{{< /xref >}}所示。

{{< fig num="10.4" src="/img/fig-10-04.png" caption="原始数据库集簇和恢复数据库集簇之间WAL段文件的关系" alt="图10.4 原始数据库集簇和恢复数据库集簇之间WAL段文件的关系" />}}
...
通过这种方式，每个时间线历史文件都会告诉我们每个恢复所得的数据库集簇的完整历史，而且它也会在PITR过程中使用。下一节将描述具体细节。
...
重启PostgreSQL服务器并进入PITR模式，数据库会沿着时间线标识2进行恢复。如{{< xref fig="10.5" anchor="fig-10.5" >}}图10.5{{< /xref >}}所示。

{{< fig num="10.5" src="/img/fig-10-05.png" caption="沿着时间线2将数据库恢复至12:15的状态" alt="图10.5 沿着时间线2将数据库恢复至12:15的状态" />}}
